...
**Ghi chú:** `BudgetLine.isMarker = true` nghĩa là mục con của một nhóm đã có trần cha — chỉ là mốc theo dõi. Mục 5 và 6 bỏ qua marker (đã có mục 7 lo cả nhóm).
...
describe('budget-pace', () => {
  ...
  it('chênh đúng 25 điểm phần trăm thì chưa báo (phải VƯỢT 0.25)', () => {
    // qua 12/31 = 0.3871; 0.3871 + 0.25 = 0.6371 → spent = 40_000 * 0.6371
    const out = budgetRules(
...
```ts
import { describe, expect, it } from 'vitest'
import { ANOMALY_FACTOR, expenseMedian, isUnusuallyLarge } from './anomaly'
