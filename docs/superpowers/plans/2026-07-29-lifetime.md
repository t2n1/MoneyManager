...
1. **Kịch bản** — tên, năm sinh (ghi vào `profiles.birth_year`), tuổi kết thúc, tiền hiển thị, lợi suất thực (%), độ rộng dải (%), công tắc "Tính theo giá danh nghĩa". Công tắc này ở đây chứ không trên header vì bật tắt nó là mọi con số trên màn đổi hết — không nên cách một cú chạm hớ hênh.
   ...
   **Đổi `display_currency` phải nói ngay hậu quả.** Đổi đơn vị hiển thị làm mọi
   `fx_to_display` đã lưu thành rác: một chặng/sự kiện từng cùng tiền với đơn vị hiển thị
   (nên tỷ giá `1` là đúng, và ô nhập bị ẩn) giờ khác tiền, mà `1` vẫn nằm đó. Ngay khi
   người dùng đổi ô này, hiện một dòng thông báo **liệt kê các chặng/sự kiện bị ảnh
   hưởng** để họ biết phải khai lại tỷ giá cho những dòng nào. Không chặn lưu — xem quyết
   định đã chốt ở Task 7 Step 2.
