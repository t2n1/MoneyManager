...
**Interfaces:**
- Consumes: `TransactionRow`, `CategoryRow` từ `../../types/database.types`; `CurrencyCode`
- Produces:
  ...
  - `suggestBaseline(txs: TransactionRow[], categories: CategoryRow[], currency: CurrencyCode, todayISO: string): BaselineSuggestion`

Chỉ nhận giao dịch **cùng một loại tiền** — caller lọc trước. Lý do: trộn tiền ở đây thì phải kéo `Rates` vào một module đang là ứng viên module lá; giả định của chặng vốn dĩ theo tiền bản địa.
...
2. `scenarios.length === 0` → nút "Tạo kịch bản từ chi tiêu thật của tôi", gọi `suggestBaseline` rồi `createLifeScenario` + `createLifePhase`. Đây là chỗ thay cho wizard.
...
4. **Số này ở đâu ra** — `suggestBaseline` của chặng hiện tại: câu "Chi X/năm lấy từ N tháng gần nhất", một thanh tỷ trọng, và ba danh mục lớn nhất. Khối này **luôn mở**, không phải bấm mới hiện — số nền sai thì cả bản chiếu 60 năm sai theo.
