...
Thêm `'features/lifetime/project.ts'` vào `ENTRY_POINTS` của
`src/features/notifications/purity.test.ts`, kèm comment nói vì sao. Không chờ Task 12:
tới đó thì đã có 9 task viết thêm code quanh `project.ts` mà ràng buộc "module lá" không
có chốt tự động nào canh — một dòng bây giờ rẻ hơn một lần review thủ công mỗi task.
...
ngoài; sửa `project.ts`, không sửa phép thử.
