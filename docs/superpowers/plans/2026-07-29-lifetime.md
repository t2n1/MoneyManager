...
(Với 3 test cuối thêm sau khi sửa lỗi thứ 11: chạy RIÊNG từng test đó với implementation
CŨ — tức bản chỉ có `ctx.fxToDisplay` không có `fxOf`/`displayCurrency` — trước khi sửa,
để thấy đỏ thật. Dùng sentinel khác `1` cho `ctx.fxToDisplay` trong ctx của test như trên;
nếu để `ctx.fxToDisplay` trùng giá trị mong đợi (`1`) thì test cũ vô tình xanh dù logic vẫn
sai — đã gặp đúng bẫy này khi làm thật, xem báo cáo Task 6.)
