...
- `EventFormSheet`: `end_year` phải null hoặc `>= start_year`; `amount_minor >= 0`; `fx_to_display > 0`. Ô tỷ giá chỉ hiện khi tiền của sự kiện **khác** `display_currency` — đó là lúc duy nhất nó được dùng tới, và cũng là lúc bắt buộc phải hỏi: tiền của sự kiện có thể khác cả tiền của chặng (年金 giữ ¥ trong khi chặng Mỹ dùng $), nên không có tỷ giá nào của chặng suy ra được nó.
