...
- Produces:
  ...
  - `interface PresetContext { scenarioId, year, currency, country, currentIncomeMinor, currentExpenseMinor, fxToDisplay, displayCurrency, fxOf }`
    (⚠️ đã sửa sau lỗi thứ 11 của plan — xem hộp cảnh báo ngay dưới)
  - `interface PresetResult { phases: NewLifePhase[]; events: NewLifeEvent[] }`

> **Lỗi thứ 11 của plan (đã sửa):** bản gốc chỉ có MỘT `fxToDisplay: number` gắn với
> `ctx.currency` (tiền của chặng đang hiệu lực). Nhưng 3 mẫu bên dưới (sinh-con,
> nghỉ-hưu, hỗ-trợ-bố-mẹ) ép cứng `currency` của SỰ KIỆN sang JPY/VND — khác hẳn
> `ctx.currency` khi kịch bản dùng tiền khác (vd USD). `fx_to_display` phải là tỷ giá
> của TIỀN SỰ KIỆN, không phải tiền chặng, nên `ctx.fxToDisplay` không đủ. Đã thêm:
> - `displayCurrency: CurrencyCode` — tiền hiển thị của kịch bản.
> - `fxOf: (currency: CurrencyCode) => number | null` — tra tỷ giá MAJOR-sang-MAJOR
>   từ một tiền bất kỳ sang `displayCurrency`, trả `null` nếu không tra được.
>
> Mỗi sự kiện tính `fx_to_display` theo tiền CỦA CHÍNH NÓ: trùng `displayCurrency` thì
> `1` (đúng và vô hại — `convertLifetimeMinor` short-circuit ở `from === to`); khác thì
> gọi `fxOf`, và nếu `fxOf` trả `null` thì **CỐ Ý** đặt `1` — không phải bỏ cuộc. Lý do:
> tổ hợp `currency !== display_currency && fx_to_display === 1` là đúng điều kiện banner
> ở Task 7 và dấu cảnh báo ở Task 11 dùng để phát hiện dòng "chưa ai khai tỷ giá". Một
> tỷ giá đoán bừa khác `1` sẽ NGẤM NGẦM qua lưới cảnh báo đó — sai một cách không ai
> thấy còn tệ hơn sai một cách lộ liễu. Nguồn cho `fxOf` là việc của **Task 11** (nơi
> dựng `ctx` thật), không phải của `presets.ts` — xem ghi chú ở Task 11 bên dưới.
...
(Với 3 test về `fxOf` thêm sau khi sửa lỗi thứ 11: chạy RIÊNG từng test đó với
implementation CŨ — tức bản chỉ có `ctx.fxToDisplay` không có `fxOf`/`displayCurrency` —
trước khi sửa, để thấy đỏ thật. Dùng sentinel khác `1` cho `ctx.fxToDisplay` trong ctx của
test như trên; nếu để `ctx.fxToDisplay` trùng giá trị mong đợi (`1`) thì test cũ vô tình
xanh dù logic vẫn sai — đã gặp đúng bẫy này khi làm thật, xem báo cáo Task 6.

Với 2 lỗi Important bắt sau đó (児童手当 ghim nhầm mốc 15 tuổi thay vì 18 sau cải cách
10/2024, và chồng lấn một năm giữa bậc "16–18 tuổi" và "đại học"): sửa assertion
`tro.end_year` sang `2029 + 18` và thêm test khẳng định các bậc tuổi không chồng lấn/
không hở TRƯỚC khi sửa `presets.ts`, chạy thấy đỏ thật (RED) rồi mới sửa code.)
...
export interface PresetContext {
   ...
   *
   * Nguồn số: việc của caller (dựng ctx — Task 11), không phải của presets.ts. Điểm
   * khởi đầu hợp lý là tỷ giá "hôm nay" từ `src/lib/rates.ts` (đã có sẵn fetch +
   * cache) — đó vẫn chỉ là một giả định, nên `note` của sự kiện luôn nhắc "kiểm tra
   * lại".
   */
...
> **Sửa sau review (3 lỗi Important + 1 quyết định Minor, cùng đợt với lỗi thứ 11):**
> 1. `end_year: ctx.year + 15` cho 児童手当 nói ngược với comment "tới hết cấp ba" — đó là
>    luật CŨ trước cải cách 10/2024 (cải cách bỏ ngưỡng thu nhập, kéo dài tới 18 tuổi).
>    Đã sửa thành `ctx.year + JIDO_TEATE_END_AGE` (= 18), comment ghi rõ mốc cải cách.
> 2. "Nuôi con 16–18 tuổi" (`end_year: +18`) chồng lấn một năm với "Con vào đại học"
>    (`start_year: +18`) — năm đó bị tính chi phí hai lần. Đã đổi bậc trước thành
>    "16–17 tuổi" (`end_year: +17`), giữ đại học `+18`→`+21`. Bốn bậc 0–6 / 7–15 / 16–17 /
>    đại học giờ khớp khít liên tiếp — không chồng lấn, không hở năm nào (có test riêng
>    khẳng định điều này, xem Step 1).
> 3. Phần lớn số mặc định (chi phí cưới, trả trước mua nhà, khoản trả vay + thời hạn vay,
>    lương hưu, chi phí chuyển nhà, hỗ trợ bố mẹ + thời hạn 20 năm) thiếu comment nguồn +
>    ngày tra. Vì KHÔNG có nguồn thật đã tra cho các số này, comment ghi thẳng "Ước lượng,
>    chưa tra nguồn (2026-07-29)" — trung thực còn hơn bịa một trích dẫn nghe có vẻ thật.
> 4. Ba khoản chi một lần (chi phí cưới, trả trước mua nhà, chi phí chuyển nhà) đổi
>    `inflate: false` → `true`: chúng là giá HÔM NAY cho việc xảy ra ở năm tương lai, nên
>    phải phồng theo lạm phát ở chế độ danh nghĩa như mọi giá cả khác. Khoản trả vay giữ
>    `false` (lãi cố định là số danh nghĩa); trợ cấp + lương hưu giữ `false` (luật ấn
>    định). Comment tại mỗi chỗ ghi rõ lý do để phân biệt hai loại — đừng gộp chung.
...
- Create: `src/features/lifetime/LifetimeChartCard.tsx` (STUB — trả `null`, Task 8 điền thân hàm)
- Create: `src/features/lifetime/InsightCards.tsx` (STUB — trả `null`, Task 9 điền thân hàm)
- Create: `src/features/lifetime/ScenarioEditorSheet.tsx` (STUB — trả `null`, giữ đúng chữ ký
  props `scenario phases events onClose` mà Task 11 mô tả; Task 11 THAY THÂN HÀM của
  đúng file này, không tạo file mới. Lỗi thứ 12 của kế hoạch: bản đầu quên liệt kê
  component này vào danh sách "component tạm" dù banner cảnh báo tỷ giá và nút bút chì
  của Task 7 đều phải trỏ tới nó — không stub thì hai chỗ đó không có gì để mở.)
- Modify: `src/App.tsx`
...
2. `scenarios.length === 0` → nút "Tạo kịch bản từ chi tiêu thật của tôi", gọi `suggestBaseline` rồi `createLifeScenario` + `createLifePhase`. Đây là chỗ thay cho wizard. Nhớ dựng `currencyOf` theo đúng pattern sẵn có trong repo (vd. `LedgerPage.tsx`: `(id) => accountOf(id)?.currency ?? base`) rồi truyền vào `suggestBaseline` — hàm không tự suy ra tiền từ `TransactionRow` (xem lại Task 5).
   ...
   **Lỗi thứ 13 của kế hoạch — `starting_assets_minor` PHẢI lấy từ tài sản ròng hiện tại, không
   được để `0`.** Bản thiết kế (`docs/superpowers/specs/2026-07-29-lifetime-design.md`) nói rõ tài
   sản khởi điểm lấy từ tài sản ròng hiện tại, nhưng `BaselineSuggestion` (Task 5) chỉ khai
   `annualIncomeMinor`/`annualExpenseMinor`/`monthsCovered`/`byCategory` — không có tài sản khởi
   điểm, nên chỉ dùng `suggestBaseline` là không đủ nguồn. Hậu quả nếu bỏ qua: bản chiếu bắt đầu từ
   0 đồng thì cả bốn thẻ kết luận (Task 9) đều sai, và người mở lần đầu thấy một đồ thị nói mình
   sắp cạn tiền dù đang có tiền — ấn tượng đầu tiên tệ nhất có thể.

   Lấy đúng công thức tài sản ròng mà `src/features/assets/AssetsPage.tsx` đã dùng (biến
   `netWorth = breakdown.total + debts_.net + breakdown.cardDebt`, và cờ `netWorthReliable`) —
   gọi lại ĐÚNG `assetBreakdown` (`features/assets/aggregate.ts`) và `debtSummary`
   (`features/debts/aggregate.ts`) với cùng input (`balances`, `base`, `rates`, `settings` đã map
   từ `asset_group_settings`, `debts`, `debtPayments`), KHÔNG tự cộng lại từ đầu — hai lối tính
   khác nhau sẽ trôi lệch nhau theo thời gian và người dùng thấy hai trang báo hai số khác nhau.

   Xử lý ca không đáng tin (thiếu tỷ giá cho một phần tài khoản/công nợ → tổng bị thiếu ÂM THẦM,
   xem `hasMissingRate`/`cardHasMissingRate`/`debts.hasMissingRate`): đáng tin → điền thẳng
   `netWorth` vào `starting_assets_minor`; không đáng tin → điền `0` **và hiện một dòng chữ ngay
   tại chỗ tạo kịch bản** nói rõ một phần tài khoản chưa quy đổi được nên phải tự nhập lại tài sản
   khởi điểm sau. Đừng điền một con số thiếu rồi im lặng — đó đúng là lớp lỗi cả đợt Lifetime đang
   đi vá (cùng họ với banner tỷ giá bằng 1 ở dưới).
...
1. **Lịch sử thật vẽ liền nét, bản chiếu vẽ nét đứt.** Lịch sử lấy từ `networth_snapshots`. Nó ngắn ngủn so với 58 năm phỏng đoán và đồ thị phải để lộ đúng tỉ lệ đó.
   ...
   **Bẫy phát hiện khi làm Task 8 (không phải giả thuyết — sẽ THẬT một khi Task 11 mở khoá đổi `display_currency`):** `networth_snapshots.net_worth` luôn ghi ở **base currency của profile**, KHÔNG phải `display_currency` của kịch bản đang xem (xem `NetWorthHistorySection.tsx` — nhận prop `base` = base currency, không phải display currency của một kịch bản cụ thể). Hôm nay hai giá trị này luôn trùng nhau vì đường DUY NHẤT tạo kịch bản (`ensureFirstScenario`) đặt `display_currency = profile.base_currency`, nhưng Task 11 sẽ cho đổi `display_currency` sau khi tạo. Lúc đó nếu vẫn vẽ thẳng `historyRows` lên trục của kịch bản, đó là vẽ SỐ CỦA MỘT ĐƠN VỊ TIỀN LÊN TRỤC CỦA ĐƠN VỊ KHÁC, sai im lặng — đúng lớp lỗi mục C1 (fx bằng 1 mặc định) mà cả đợt Lifetime đang vá.

   `LifetimeChartCard` phải nhận thêm một prop `historyCurrency: CurrencyCode` (base currency của profile, LifetimePage truyền vào) để tự so với `currency` (display_currency của kịch bản). Lệch nhau thì **ẩn hẳn đường lịch sử** (không quy đổi — không có tỷ giá giả định nào cho chuỗi lịch sử để tự bịa ra một cái) và thay legend + `aria-label` bằng một câu ngắn nói RÕ lý do ẩn ("khác đơn vị tiền"). Đừng ẩn lặng thinh: ẩn lặng thì người dùng tưởng chưa có dữ liệu lịch sử và đi tìm hiểu tại sao.
...
Dưới đồ thị, chế độ so sánh hiện một dòng lấy từ `compareAtEnd(rows, compareRows)` — **NHƯNG chỉ khi hai kịch bản CÙNG `display_currency`.**

**Sửa sau khi Task 8 làm xong lần đầu:** bản đầu viết câu rào "phụ thuộc tỷ giá giả định" khi hai kịch bản khác tiền tệ, nhưng đó là kê nửa liều. Đọc lại `compareAtEnd` (Task 4, `insights.ts`): hàm này **không hề quy đổi tỷ giá nào cả** — chỉ trừ thẳng `a[last].assetsEndMinor − b[last].assetsEndMinor`. Nếu hai bên khác `display_currency`, hiệu đó là hiệu của HAI ĐƠN VỊ KHÁC NHAU (kiểu lấy số yên trừ thẳng số cent đô, lệch nhau hàng trăm lần chỉ vì JPY 0 chữ số thập phân còn USD 2 chữ số) — một con số RÁC, không phải "một phép trừ có giả định tỷ giá". Gắn câu rào "phụ thuộc tỷ giá giả định" vào một con số rác vẫn khiến người ta đọc nó thành kết luận, chỉ là kết luận có vẻ thận trọng hơn.

Đúng cách: khi `display_currency` của `rows` và `compareRows` **khác nhau** thì **KHÔNG tính/hiện hiệu số** — thay vào đó hiện HAI giá trị `assetsEndMinor` cuối kỳ CẠNH NHAU, mỗi cái format đúng đơn vị tiền của kịch bản đó, kèm một câu ngắn nói hai kịch bản khác đơn vị tiền nên không trừ trực tiếp được, để người dùng tự so bằng mắt. Khi CÙNG `display_currency` thì giữ nguyên hiệu số như bình thường (`compareDiff >= 0` màu xanh, âm màu đỏ) — lúc đó nó có nghĩa thật.
...
> **Sửa sau review (lỗi thứ 14 của kế hoạch):** bản đầu của bước này (viết bởi chính
> plan, không phải do implementer lệch) THIẾU BOM UTF-8 + CRLF. Tiêu đề cột cố tình
> không dấu để Excel bản Nhật/Mỹ mở không lỗi font, nhưng THÂN file (nhãn sự kiện tiếng
> Việt, tên chặng tiếng Nhật như 年金) vẫn giữ dấu — thiếu BOM thì thân vẫn hỏng font,
> đánh bại đúng mục đích đặt ra. Bản đúng dưới đây dùng chung `escapeCsv` đã export từ
> `reports/csv.ts` (thay vì viết hàm `cell()` cục bộ lệch luật escape) và trả về
> `'﻿' + lines.join('\r\n')`, khớp quy ước sẵn có của `reports/csv.ts`.
...
`reports/csv.ts` phải export `escapeCsv` (trước đó là hàm riêng không export) để module này dùng chung, tránh hai luật escape lệch nhau theo thời gian.
...
Năm có `assetsPessimisticMinor < 0` được đánh dấu `border-l-[3px] border-red-600 bg-red-50 dark:bg-red-900/20` và **có icon** `AlertCircle` — không chỉ dựa vào màu. Trên `<table>` thật, đặt nền đỏ ở `<tr>` nhưng border trái đặt ở `<td>` đầu tiên — trình duyệt không vẽ border trên `<tr>` đáng tin cậy dưới `border-collapse`.
...
Nút xuất CSV gọi `downloadTextFile(\`lifetime-${scenarioName}.csv\`, buildYearCsv(rows, currency), 'text/csv')`. `scenarioName` là một prop bổ sung (brief gốc không liệt kê trong "Interfaces" nhưng có dùng biến này) — optional, dùng để đặt tên file cho dễ phân biệt kịch bản; dùng `normalizeText` (`features/transactions/filter.ts`) để bỏ dấu trước khi rút gọn, tránh tên file kiểu "k-ch-b-n-c-a-t-i".

**Bắt buộc đóng được bằng phím Esc**, cùng quy ước mọi dialog/sheet khác trong app (`lib/dialog.tsx`, `NotificationBell.tsx`): `useEffect` gắn `window.addEventListener('keydown', ...)`, dọn listener khi unmount. Component này CHÍNH LÀ bản dự phòng a11y của đồ thị — không được để người dùng bàn phím bị kẹt lại trong chính bản dự phòng đó.
...
Ghi kết quả đo vào commit message — đo đúng phần tử chứa các dòng ĐÃ hiện đủ (đợi state cập nhật xong trước khi đọc `scrollHeight`), không đo nhầm khối bọc ngoài còn đang lọc.
...
**Files:**
- Modify: `src/features/lifetime/ScenarioEditorSheet.tsx` (Task 7 đã tạo file này làm STUB
  trả `null` với đúng chữ ký props `scenario phases events onClose` — việc của Task 11 là
  THAY THÂN HÀM bên trong, không tạo file mới, và `LifetimePage` đã nối dây gọi component
  này rồi nên không cần sửa lại chỗ gọi.)
- Create: `src/features/lifetime/PhaseFormSheet.tsx`
- Create: `src/features/lifetime/EventFormSheet.tsx`
- Modify: `src/features/lifetime/LifetimePage.tsx` (nếu cần — phần lớn việc nối dây header/banner đã xong ở Task 7)
...
1. **Kịch bản** — tên, năm sinh (ghi vào `profiles.birth_year`), tuổi kết thúc, tiền hiển thị,
   **tài sản khởi điểm**, lợi suất thực (%), độ rộng dải (%), công tắc "Tính theo giá danh nghĩa".
   Công tắc này ở đây chứ không trên header vì bật tắt nó là mọi con số trên màn đổi hết — không
   nên cách một cú chạm hớ hênh.

   Ô **tài sản khởi điểm** là chỗ người dùng sửa lại con số Task 7 đã tự điền (tài sản ròng hiện
   tại tại thời điểm tạo kịch bản, hoặc `0` nếu lúc đó thiếu tỷ giá — xem lỗi thứ 13 ở Task 7).
   Con số đó chỉ đúng tại đúng lúc bấm nút tạo; tài sản ròng đổi theo thời gian nên ô này CẦN sửa
   được tay, không phải trường chỉ-đọc.
   ...
   **Hệ quả tới đồ thị (Task 8) — biết trước khi bật cho phép đổi `display_currency`:**
   `LifetimeChartCard` nhận `historyCurrency` (base currency của profile) và so với
   `display_currency` của kịch bản để quyết định có vẽ đường "lịch sử thật"
   (`networth_snapshots`) hay không — hai giá trị này LUÔN trùng nhau ở mọi kịch bản tạo
   qua `ensureFirstScenario` (Task 7), nhưng Task 11 chính là chỗ MỞ KHOÁ cho chúng lệch
   nhau lần đầu tiên (người dùng đổi `display_currency` sau khi tạo). Từ lúc Task 11 lên,
   một kịch bản có `display_currency` khác `profiles.base_currency` sẽ khiến đồ thị tự ẩn
   đường lịch sử và hiện dòng giải thích thay vì vẽ — đây là hành vi ĐÚNG CHỦ Ý của Task 8,
   không phải bug cần vá ở Task 11. Không cần làm gì thêm cho ca này ở đây, chỉ cần biết để
   không tưởng nhầm là lỗi khi thấy đồ thị "mất" đường lịch sử sau khi đổi tiền hiển thị.
...
**Dựng `ctx: PresetContext` — đây là chỗ DUY NHẤT trong cả tính năng chịu trách nhiệm
điền `displayCurrency` và `fxOf` (đã sửa ở Task 6 sau lỗi thứ 11 của plan: `PresetContext`
bản đầu chỉ có một `fxToDisplay` vô hướng, không đủ cho sự kiện mang tiền khác chặng).**

- `displayCurrency`: lấy thẳng từ `scenario.display_currency` (đã có sẵn trên state của
  `ScenarioEditorSheet`).
- `fxOf`: dựng từ tỷ giá "hôm nay" — gọi `fetchRates(scenario.display_currency)` ở
  `src/lib/rates.ts` (đã có sẵn fetch + cache localStorage, xem Task 7/8 nếu đã có chỗ gọi
  tương tự để tái dùng thay vì gọi lại), rồi bọc kết quả thành hàm:

  ```ts
  const rates = await fetchRates(scenario.display_currency) // Rates: base→X, X là target
  const fxOf = (currency: CurrencyCode): number | null => {
    if (currency === scenario.display_currency) return 1
    const rate = rates[currency]
    // Rates ở lib/rates.ts là "1 base đổi được rates[X] đơn vị X" — CHIỀU NGƯỢC với
    // fx_to_display ("1 currency đổi được ? display"), nên phải nghịch đảo (1 / rate),
    // không được gán thẳng. Nhầm chỗ này là lặp lại đúng lỗi 22.500 lần đã cảnh báo ở
    // Step 2 dưới, chỉ khác là lỗi nằm trong code thay vì trong ô người dùng gõ tay.
    return rate ? 1 / rate : null
  }
  ```

  `fetchRates` là async và có thể lỗi mạng (throw nếu chưa từng cache) — gọi trước khi mở
  sheet chọn mẫu (hoặc cache kết quả ở `useLifetime`/`ScenarioEditorSheet` state), không
  gọi đồng bộ trong lúc dựng `ctx`. Nếu `fetchRates` throw (lần đầu chưa có cache, mất
  mạng), cho `fxOf` luôn trả `null` — mẫu vẫn sinh được bản ghi, chỉ là mọi sự kiện khác
  tiền hiển thị sẽ mang `fx_to_display = 1` và bị banner Task 7 + dấu cảnh báo Step 2 bắt
  ngay, đúng như thiết kế "sai một cách nhìn thấy được".
- Con số từ `fxOf` vẫn chỉ là một GIẢ ĐỊNH (tỷ giá thị trường hôm nay, không phải tỷ giá
  người dùng sẽ dùng thật khi sự kiện xảy ra ở tương lai) — `note` của bản ghi mẫu đã tự
  mang chuỗi "Số mặc định, kiểm tra lại" nên không cần thêm cảnh báo riêng ở bước này,
  nhưng ĐỪNG hiểu nhầm `fxOf` trả được số là "đã đúng, khỏi cần dòng quy đổi ở Step 2" —
  dòng quy đổi vẫn phải hiện cho mọi sự kiện khác `display_currency`, kể cả sự kiện sinh
  từ mẫu.
