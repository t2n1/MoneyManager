...
**Interfaces:**
- Consumes: `TransactionRow`, `CategoryRow` từ `../../types/database.types`; `CurrencyCode`; `CurrencyOf` (đã có sẵn ở `features/reports/aggregate.ts`, cùng chữ ký với bản ở `features/transactions/ledgerShared.ts` — dùng lại, đừng khai kiểu mới)
- Produces:
