...
và gán vào 3 khóa mới của `BackupData`.
...
- [ ] **Step 5: Kiểm tra module lá**

Run: `npx vitest run src/features/notifications/purity.test.ts`
Expected: PASS (chưa nối vào bộ luật nên chưa thể đỏ, nhưng chạy để có mốc so sánh ở Task 12)
...
describe('firstNegativeYear', () => {
  ...
  it('nhánh thấp âm sớm hơn nhánh trung tâm', () => {
    // Nghỉ hưu ngay: không thu, chi đều, tài sản tụt dần. Nhánh thấp tụt nhanh hơn.
    const rows = rowsOf({
 ...
 * `branch` 'low' đọc nhánh lợi suất thấp — con số đáng lo hơn và là thứ thẻ kết
 * luận hiện, vì mốc âm của nhánh trung tâm dịch cả chục năm khi đổi lợi suất 1%.
 */
export function firstNegativeYear(rows: YearRow[], branch: 'center' | 'low'): number | null {
  for (const r of rows) {
    const v = branch === 'low' ? r.assetsLowMinor : r.assetsEndMinor
...
export function assetsAtAge(
  ...
  if (!row) return null
  return { center: row.assetsEndMinor, low: row.assetsLowMinor, high: row.assetsHighMinor }
}
...
2. **Dải dao động** thay cho một đường sắc nét: `<Area>` với `dataKey` trả cặp `[assetsLowMinor, assetsHighMinor]`, `fillOpacity={0.13}`, không có `stroke`.
...
Bọc `ResponsiveContainer` trong một `<div role="img" aria-label={...}>` với câu mô tả sinh từ dữ liệu, ví dụ: `Tài sản ròng từ 2026 đến 2084. Nhánh trung tâm đạt đỉnh 150 triệu quanh 2056 rồi về 82 triệu. Nhánh thấp âm từ 2079.` Đồ thị một mình không đọc được, nên câu này là bắt buộc.
...
const row: YearRow = {
  ...
  assetsLowMinor: 20_000_000,
  assetsHighMinor: 22_000_000,
}
...
describe('buildYearCsv', () => {
  it('dòng đầu là tiêu đề cột', () => {
    ...
    expect(csv.split('\n')[0]).toBe(
      'Nam,Tuoi,Noi o,Thu,Chi,Su kien,Tai san cuoi nam,Nhanh thap,Nhanh cao',
    )
  })
...
const HEADER = 'Nam,Tuoi,Noi o,Thu,Chi,Su kien,Tai san cuoi nam,Nhanh thap,Nhanh cao'
  ...
  for (const r of rows) {
    lines.push(
      [
        ...
        minorToPlain(r.assetsLowMinor, currency),
        minorToPlain(r.assetsHighMinor, currency),
      ].join(','),
    )
  }
  return lines.join('\n')
...
Năm có `assetsLowMinor < 0` được đánh dấu `border-l-[3px] border-red-600 bg-red-50 dark:bg-red-900/20` và **có icon** `AlertCircle` — không chỉ dựa vào màu.
...
3. **Sự kiện** — danh sách sắp theo `start_year`, mỗi dòng hiện label, khoảng năm, và số tiền **kèm loại tiền riêng của dòng đó**. Bắt buộc hiện loại tiền: 年金 vẫn là ¥ trong khi chặng Mỹ dùng $, không hiện thì đọc nhầm.
...
- `EventFormSheet`: `end_year` phải null hoặc `>= start_year`; `amount_minor >= 0`.
...
- [ ] Áp migration `0031_lifetime.sql` lên Supabase thật. Các migration trước có tiền lệ bị quên bước này (xem `docs/superpowers/specs/`), nên **không coi tính năng là xong** khi chưa áp.
