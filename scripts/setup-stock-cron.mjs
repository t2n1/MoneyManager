// Hẹn cron cho `stock-refresh`: nhận secret ở terminal của chủ app, CHỨNG MINH secret đó
...
//
// Chạy:
//   node scripts/setup-stock-cron.mjs              (hỏi secret, không hiện lên màn hình)
//   npm run setup:stock-cron
//
// Kiểm script mà không cần secret, không gọi mạng:
//   node scripts/setup-stock-cron.mjs --dry-run
//
...
//   node scripts/setup-stock-cron.mjs --khong-goi
//
...
/** 08:45 UTC = 15:45 giờ VN, thứ Hai–thứ Sáu: sau khi sàn đóng (15:00) và khớp ATC xong. */
const LICH = '45 8 * * 1-5'
const TEN_JOB = 'stock-refresh-daily'
/**
 * Phải lớn hơn FETCH_BUDGET_MS (90s) trong supabase/functions/stock-refresh/prices.ts,
 ...
 * function lúc nó mới hút được vài lô.
 */
...
async function goiThu(ref, secret) {
  const url = `https://${ref}.supabase.co/functions/v1/stock-refresh`
...
console.log(`
Hẹn cron '${TEN_JOB}' (${LICH} — 15:45 giờ VN, thứ Hai–thứ Sáu) cho project ${ref}.
...
push-notify đang dùng — nó là "bí mật cho cron" nói chung).
`)
