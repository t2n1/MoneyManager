...
// `TransactionRow` thật không có cột `currency` (tiền suy ra từ tài khoản nguồn,
// xem baseline.ts). Mở rộng kiểu tham số CHỈ ở test để gắn tạm field này cho các
// ca cố ý trộn tiền — không phản ánh dữ liệu thật, xem baseline.ts để biết vì sao.
function tx(over: Partial<TransactionRow> & { currency?: CurrencyCode }): TransactionRow {
  return {
    id: 'x',
    user_id: 'u',
...
describe('suggestBaseline', () => {
  it('quy năm hoá thu và chi trong 12 tháng gần nhất', () => {
    ...
    const s = suggestBaseline(txs, cats, 'JPY', '2026-07-29')
  ...
  it('bỏ giao dịch khác loại tiền', () => {
    const txs = [
      tx({ id: '1', amount: 900_000, currency: 'USD' }),
      tx({ id: '2', amount: 100_000, currency: 'JPY' }),
    ]
    const s = suggestBaseline(txs, cats, 'JPY', '2026-07-29')
    expect(s.annualExpenseMinor).toBe(100_000)
  })
  ...
  it('không có giao dịch nào thì trả 0, không chia cho 0', () => {
    const s = suggestBaseline([], cats, 'JPY', '2026-07-29')
