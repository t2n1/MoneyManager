  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="grid grid-cols-3 gap-2 rounded-xl bg-white p-3 text-center shadow-sm">
        <div>
          <div className="text-xs text-gray-500">Thu cả năm</div>
        ...
        <div className="border-x border-gray-100">
          <div className="text-xs text-gray-500">Chi cả năm</div>
        ...
        <div>
          ...
          <div
            className={`mt-0.5 text-sm font-semibold tabular-nums ${yearIncome - yearExpense < 0 ? 'text-red-600' : 'text-gray-800'}`}
          >
            {approx}
      ...
      {isLoading && !active ? (
        ...
        <div className="overflow-hidden rounded-xl bg-white shadow-sm">
          <div className="grid grid-cols-[auto_1fr_1fr_1fr] gap-2 border-b border-gray-100 px-3 py-2 text-[11px] font-medium uppercase tracking-wide text-gray-400">
            <span>Tháng</span>
            ...
            {points.map((p) => {
              ...
              return (
                <button
                  ...
                  className="grid w-full grid-cols-[auto_1fr_1fr_1fr] items-center gap-2 px-3 py-2.5 text-right text-sm transition hover:bg-gray-50 active:bg-gray-100"
                >
                  <span className="text-left font-medium text-gray-700">Th {p.key.month}</span>
                  <span className={`tabular-nums ${empty ? 'text-gray-300' : 'text-green-600'}`}>
                    {p.income > 0 ? formatMoney(p.income, base) : '–'}
                  </span>
                  <span className={`tabular-nums ${empty ? 'text-gray-300' : 'text-red-500'}`}>
                    {p.expense > 0 ? formatMoney(p.expense, base) : '–'}
                  </span>
                  <span
                    className={`tabular-nums font-medium ${
                      empty ? 'text-gray-300' : net < 0 ? 'text-red-600' : 'text-gray-800'
                    }`}
                  >
